...
The data is now in a â€˜tibbleâ€™ (like a spreadsheet for R). Letâ€™s use the `head` function to look at the first few rows:  
...
We will be using the very useful `cumsum` function (short for â€˜cumulative sumâ€™) to calculate the cumulative income. To see what this function does, look at this simple example.  
...
`ggplot` assumes that the different lines you want to show are identified through the different values in one variable (here, the `Country` variable). If your data is formatted differently, for example, if you have one variable for the Gini of each country (â€˜wideâ€™ format), then in order to use `ggplot` you will first have to transform the dataset into â€˜longâ€™ format. Doing so is beyond the scope of this task, however you can find a worked example online, such as 'R TSplots' (https://tinyco.re/5093147).[^1] Project 4 also explains how to transform data between â€˜wideâ€™ and â€˜longâ€™ formats. 

[^1]: University of Manchesterâ€™s Econometric Computing Learning Resource (ECLR). 2018. â€˜R TSplotsâ€™. Updated 26 July 2016.
...
The variable `Entity` is the country and the variable `X.percent` is the health Gini. Letâ€™s change these variable names (to `Country` and `Health`, respectively) to clarify what they actually refer to, which will help when writing code (and if we go back to read this code at a later date). 
...
As shown in R walk-through 5.7, the data is already formatted so that we can use `ggplot` directly (in â€˜longâ€™ format), in other words we have only one variable for the mortality Gini (`HGini`), and we can separate the data by country using one variable (`Country`). 
...
A blog post (http://tinyco.re/1992106) from *Data Se* provides the following code for â€˜R geom_bar change orderâ€™, and uses the `reorder` function to reorder the horizontal axis variable (`Code`) according to the `HGini` value.
...
Letâ€™s change the names of these variables (to `Private_Access` and `Public_Access` respectively) to make working with them easier: 
...
To find details about these variables, click the â€˜Metadataâ€™ button on the website to find the following explanation (under â€˜Method of measurementâ€™): 
...
From the summary statistics for the `Public_Access` variable, you can see that there are two missing observations (`NA`). Here, we will keep these observations because leaving them in doesnâ€™t affect the following analysis. 
...
Letâ€™s find the extreme values, starting with the two countries where public sector patients have access to all (100%) essential medications (which you can also see in the chart). 
...
As usual, ensure that you understand the definition of the variables you are using. In the Our World in Data website, look at the â€˜Sourcesâ€™ tab underneath the graph for a definition:
...
You can find nearly all the countries in the world in this list (plus some sub- and supra-country entities, like OECD countries, which explains why the variable wasn't initially called â€˜Countryâ€™).
...
ggplot(temp_data2, aes(
  ...
  labs(title = "Change (%) in female pupilsâ€™ share of 
    total enrolment in primary education",
